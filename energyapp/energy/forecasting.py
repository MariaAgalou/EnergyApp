...
def forecastingprocess(steps, flag, day=True, week=False, month=False):
    ...
    if flag == True:
        ...
        split_index = len(currlist)
        segment1 = currdf[currdf.index <= split_index+1]
        segment2 = currdf[currdf.index > split_index]

        # Interactive graph
        fig = px.line(segment1, x=segment1.index, y='value', line_shape='linear', 
              labels={'index': 'Time', 'y': 'KWH'}, color_discrete_sequence=['blue'])
        
        fig.add_trace(px.line(segment2, x=segment2.index, y='value', line_shape='linear', 
